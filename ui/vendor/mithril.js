...
Vnode.normalizeChildren = function (input) {
  const children0 = [];
  if (input.length) {
    const isKeyed = input[0] != null && input[0].key != null;
...
// }
const hyperscriptVnode = function () {
  let attrs1 = arguments[this]; let start = this + 1; let children1;
  if (attrs1 == null) {
    attrs1 = {};
...
function compileSelector (selector) {
  let match; let tag = 'div'; const classes = []; const attrs = {};
  while (match = selectorParser.exec(selector)) {
    const type = match[1]; const value = match[2];
...
function execSelector (state, vnode) {
  ...
  const children = Vnode.normalizeChildren(vnode.children);
  const hasClass = hasOwn.call(attrs, 'class');
  const className = hasClass ? attrs.class : attrs.className;
  vnode.tag = state.tag;
  vnode.attrs = null;
...
function hyperscript (selector) {
  ...
  const vnode = hyperscriptVnode.apply(1, arguments);
...
hyperscript.fragment = function () {
  const vnode2 = hyperscriptVnode.apply(0, arguments);
  vnode2.tag = '[';
...
var PromisePolyfill = function (executor) {
  ...
  const self = this; const resolvers = []; const rejectors = []; const resolveCurrent = handler(resolvers, true); const rejectCurrent = handler(rejectors, false);
...
PromisePolyfill.prototype.then = function (onFulfilled, onRejection) {
  const self = this; const instance = self._instance;
  ...
  const promise = new PromisePolyfill(function (resolve, reject) { resolveNext = resolve, rejectNext = reject; });
...
PromisePolyfill.all = function (list) {
  return new PromisePolyfill(function (resolve, reject) {
    const total = list.length; let count = 0; const values = [];
...
const _12 = function ($window) {
  ...
  function createHTML (parent, vnode3, ns, nextSibling) {
    const match0 = vnode3.children.match(/^\s*?<(\w+)/im) || [];
  ...
  function createFragment (parent, vnode3, hooks, ns, nextSibling) {
    ...
    if (vnode3.children != null) {
      const children3 = vnode3.children;
  ...
  function updateNodes (parent, old, vnodes, hooks, nextSibling, ns) {
    if (old === vnodes || old == null && vnodes == null) return;
      ...
      const isKeyed0 = vnodes[0] != null && vnodes[0].key != null;
      ...
      if (isOldKeyed !== isKeyed0) {
        ...
        if (start > end) removeNodes(parent, old, oldStart, oldEnd + 1);
          ...
          const originalNextSibling = nextSibling; const vnodesLength = end - start + 1; const oldIndices = new Array(vnodesLength); let li = 0; let i = 0; let pos = 2147483647; let matched = 0; var map; let lisIndices;
          for (i = 0; i < vnodesLength; i++) oldIndices[i] = -1;
          for (i = end; i >= start; i--) {
            if (map == null) map = getKeyMap(old, oldStart, oldEnd + 1);
            ve = vnodes[i];
            const oldIndex = map[ve.key];
            if (oldIndex != null) {
  ...
  function updateNode (parent, old, vnode3, hooks, nextSibling, ns) {
    const oldTag = old.tag; const tag = vnode3.tag;
    if (oldTag === tag) {
  ...
  function updateFragment (parent, old, vnode3, hooks, nextSibling, ns) {
    ...
    let domSize = 0; const children3 = vnode3.children;
    vnode3.dom = null;
    if (children3 != null) {
  ...
  function updateElement (old, vnode3, hooks, ns) {
    const element = vnode3.dom = old.dom;
    ns = getNameSpace(vnode3) || ns;
  ...
  function getKeyMap (vnodes, start, end) {
    ...
    for (; start < end; start++) {
      const vnode3 = vnodes[start];
      if (vnode3 != null) {
        const key = vnode3.key;
  ...
  function maybeSetContentEditable (vnode3) {
    ...
    if (children3 != null && children3.length === 1 && children3[0].tag === '<') {
      const content = children3[0].children;
  ...
  function removeHTML (parent, vnode3) {
    for (let i = 0; i < vnode3.instance.length; i++) {
    ...
    while (vnode3.dom != null && vnode3.dom.parentNode === parent) {
      if (typeof vnode3.tag !== 'string') {
        ...
        if (vnode3.children.length === 1) {
          ...
          for (let i = 0; i < vnode3.children.length; i++) {
            const child = vnode3.children[i];
  ...
  function setLateSelectAttrs (vnode3, attrs2) {
    if ('value' in attrs2) {
      if (attrs2.value === null) {
        ...
        const normalized = '' + attrs2.value; // eslint-disable-line no-implicit-coercion
  ...
  EventDict.prototype.handleEvent = function (ev) {
    const handler0 = this['on' + ev.type];
    let result;
...
const render = _12(window);
const _15 = function (render0, schedule, console) {
  const subscriptions = [];
  let rendering = false;
  let pending = false;
  function sync () {
...
const mountRedraw0 = _15(render, requestAnimationFrame, console);
...
const buildPathname = function (template, params) {
  if ((/:([^\/\.-]+)(\.{3})?:/).test(template)) {
  ...
  const query = {};
  assign(query, params);
  const resolved = path.replace(/:([^\/\.-]+)(\.{3})?/g, function (m2, key1, variadic) {
    delete query[key1];
  ...
  let result0 = resolved.slice(0, newPathEnd);
...
const _18 = function ($window, Promise, oncompletion) {
  let callbackCount = 0;
  ...
  function makeRequest (factory) {
    return function (url, args) {
      if (typeof url !== 'string') { args = url; url = url.url; } else if (args == null) args = {};
      ...
      function wrap (promise1) {
        ...
        promise1.then = function () {
          count++;
          const next0 = then1.apply(promise1, arguments);
  ...
  function hasHeader (args, name) {
    for (const key0 in args.headers) {
  ...
  return {
    request: makeRequest(function (url, args, resolve, reject) {
      ...
      const assumeJSON = (args.serialize == null || args.serialize === JSON.serialize) && !(body instanceof $window.FormData);
      ...
      const original0 = xhr; let replacedAbort;
...
const request = _18(window, PromisePolyfill, mountRedraw0.redraw);
const mountRedraw = mountRedraw0;
const m = function m () { return hyperscript.apply(this, arguments); };
m.m = hyperscript;
m.trust = hyperscript.trust;
m.fragment = hyperscript.fragment;
m.mount = mountRedraw.mount;
const m3 = hyperscript;
const Promise = PromisePolyfill;
const parseQueryString = function (string) {
  if (string === '' || string == null) return {};
  if (string.charAt(0) === '?') string = string.slice(1);
  const entries = string.split('&'); const counters = {}; const data0 = {};
  for (let i = 0; i < entries.length; i++) {
    ...
    for (let j0 = 0; j0 < levels.length; j0++) {
      let level = levels[j0]; const nextLevel = levels[j0 + 1];
...
const parsePathname = function (url) {
  const queryIndex0 = url.indexOf('?');
  const hashIndex0 = url.indexOf('#');
  const queryEnd0 = hashIndex0 < 0 ? url.length : hashIndex0;
  const pathEnd0 = queryIndex0 < 0 ? queryEnd0 : queryIndex0;
  let path1 = url.slice(0, pathEnd0).replace(/\/{2,}/g, '/');
  if (!path1) path1 = '/';
  else {
...
const compileTemplate = function (template) {
  ...
  const keys = [];
  const regexp = new RegExp('^' + templateData.path.replace(
  ...
  return function (data1) {
    ...
    const values = regexp.exec(data1.path);
...
const sentinel0 = {};
const _25 = function ($window, mountRedraw00) {
  let fireAsync;
  ...
  let currentResolver = sentinel0; let component; let attrs3; let currentPath; let lastUpdate;
  ...
  function route (root, defaultRoute, routes) {
    ...
    // 2 = ready
    let state = 0;
    const compiled = Object.keys(routes).map(function (route) {
    ...
    function resolveRoute () {
      ...
      const path0 = prefix.concat()
        .replace(/(?:%[a-f89][a-f0-9])+/gim, decodeURIComponent)
        .slice(route.prefix.length);
      const data = parsePathname(path0);
    ...
    return mountRedraw00.mount(root, {
      ...
      view: function () {
        ...
        let vnode5 = [Vnode(component, attrs3.key, attrs3)];
  ...
  route.Link = {
    view: function (vnode5) {
      const options = vnode5.attrs.options;
      // Remove these so they don't get overwritten
      const attrs3 = {}; let onclick; let href;
      ...
      // should honor that.
      const child0 = m3(vnode5.attrs.selector || 'a', attrs3, vnode5.children);
